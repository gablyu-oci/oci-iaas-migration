"""Central registry of agent tools + skill-dispatch workflow reference.

Single source of truth for:
  - What tools exist and what each one does.
  - Which agents (writer / reviewer) can call which tools.
  - The canonical dependency-wave guidance for skill dispatch.
...
The production migration dispatcher lives in
``app/services/plan_orchestrator.py`` — a deterministic Python child-
process orchestrator. There is no LLM orchestrator agent today; each
skill runs as an independent writer/reviewer SkillGroup invoked by the
plan orchestrator.
"""
...
ORCHESTRATOR_WORKFLOW: dict = {
    "type": (
        "Deterministic Python dispatcher (app/services/plan_orchestrator.py) "
        "spawned as a child process from the plans API. No LLM call at the "
        "orchestrator layer — only the writer/reviewer skill loops invoke LLMs."
    ),
    "loop_policy": {
    ...
    "concurrency": (
        "plan_orchestrator dispatches skills derived from the resource graph; "
        "the DEPENDENCY_WAVES list above is canonical guidance for ordering."
    ),
...
def render_registry_markdown() -> str:
    ...
    for role, meta in AGENT_ROLES.items():
        ...
        lines.append("")
    lines.append("## Dispatch workflow\n")
    ...
    lines.append(
        ...
        "Any AWS type NOT on this list is reported back from the plan "
        "orchestrator as an unknown / unmapped resource so the user sees "
