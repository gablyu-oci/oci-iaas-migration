...
These are the **skill tools** — used by writer/reviewer agents inside a
skill group. Read-only. ``terraform_validate`` runs ``terraform`` inside
a bubblewrap (``bwrap``) sandbox when available: no network, no filesystem
